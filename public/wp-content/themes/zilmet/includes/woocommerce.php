...
//Скрыть доставку курьером для стоимости заказа менее 5000 рублей
...
add_filter( 'zilmet_avaliable_methods', 'coutier_cost_depend_weight' );
function coutier_cost_depend_weight( $avaliable_methods ) {
	$weight = WC()->cart->get_cart_contents_weight();
	if ( $weight > 100 ) {
		$avaliable_methods["flat_rate:4"]->set_cost( 1500 );
	}

	return $avaliable_methods;
}

// add_filter( 'zilmet_avaliable_methods', 'rechnoy_max_weight' );
function rechnoy_max_weight( $avaliable_methods ) {
	$weight = WC()->cart->get_cart_contents_weight();
	if ( $weight > 20 ) {
		unset( $avaliable_methods["local_pickup:6"] );
	}

	return $avaliable_methods;
}

add_filter( 'zilmet_avaliable_methods', 'rechnoy_max_volume' );
function rechnoy_max_volume( $avaliable_methods ) {
	$cart_contents_tmp = WC()->cart->get_cart_contents();
	foreach ( $cart_contents_tmp as $cart_items ) {
		$vol = get_field( 'volume', $cart_items["variation_id"] );
		if ( $vol && $vol > 100 ) {
			unset( $avaliable_methods["local_pickup:6"] );
		}
	}
...
// ВОЗМОЖНОСТЬ ДОБАВЛЯТЬ ACF В АТРИБУТЫ ВАРИАЦИЙ ТОВАРА
...
// ДОБАВЛЕНИЕ СООБЩЕНИЯ ПРИ ВЫБОРЕ ДОСТАВКИ ЧЕРЕЗ ТК
add_action( 'woocommerce_customer_email_message', 'add_message_free_shipping_in_customer_mail', 10 );
function add_message_free_shipping_in_customer_mail( $order ) {
	$shipping_lines      = $order->get_data();
	$order_item_shipping = array_values( $shipping_lines["shipping_lines"] )[0];
	if ( $order_item_shipping->get_method_id() === "free_shipping" ) {
		echo "<p><b>Для отправки транспортной компанией потребуются паспортные данные грузополучателя, а также ФИО и телефон (если они отличаются от тех, которые указали при заказе).</b></p>";
	}
}